...
set ansi_padding on
go
alter table MSG_RECIPS
drop constraint FK_CONNECT_MSG_RECIPS_GRP
alter table SMS_CONNECT
drop constraint PK_SMS_CONNECT_MSG
drop table SMS_CONNECT_MSG
if not exists (select 1 from information_schema.tables where table_name = 'SMS_CONNECT_MSG')
begin
  create table dbo.SMS_CONNECT_MSG(
	...
	,constraint PK_SMS_CONNECT_MSG primary key (CONNECT_GRP)
)
end
go
